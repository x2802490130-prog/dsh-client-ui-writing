window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var inject = ["slots", "workspaces", "remote"];
    ...
    function WritingPanel(props) {
      ...
      function refresh() {
        ...
        props.libraryList().then(function (r) { if (r && r.ok) setLibrary(r.value); else setLibrary(null); }).catch(function (x) { setErr(String((x && x.message) || x)); });
        props.getCwd().then(function (root) {
          props.projectStatus(root).then(function (r) { if (r && r.ok) setProject(r.value); else setProject(null); }).catch(function () {});
          props.evolutionList(root).then(function (r) { if (r && r.ok) setEvolution(r.value || []); else setEvolution([]); }).catch(function () {});
          props.threadGraph(root).then(function (r) { if (r && r.ok) setThreads(r.value || []); else setThreads([]); }).catch(function () {});
          props.loreList(root).then(function (r) { if (r && r.ok) setLore(r.value); else setLore(null); }).catch(function () {});
          setBusy(false);
      ...
      function doSync() {
        setBusy(true); setErr(null);
        props.getCwd().then(function (root) {
          return props.syncNow(root).then(function (r) {
            if (r && r.error) { setErr(String(r.error)); setBusy(false); return; }
            refresh();
          });
      ...
      function doSearch() {
        ...
        setBusy(true); setErr(null);
        props.getCwd().then(function (root) {
          return props.search(root, query).then(function (r) { if (r && r.ok) setResults(r.value); else setResults(null); setBusy(false); });
        }).catch(function (x) { setErr(String((x && x.message) || x)); setBusy(false); });
      }
      useEffect(function () { if (open) refresh(); }, [open]);

      if (preset !== undefined && preset !== "writing") return null;
        ...
        if (!project) content = e("div", { style: { color: C.dim } },
          ...
          e("button", {
            onClick: function () {
              setBusy(true); setErr(null);
              props.getCwd().then(function (root) {
                return props.scaffoldNow(root).then(function (r) {
                  if (r && r.error) { setErr(String(r.error)); setBusy(false); return; }
                  refresh();
                });
              }).catch(function (x) { setErr(String((x && x.message) || x)); setBusy(false); });
      ...
      var backdrop = open ? e("div", { onClick: function () { setOpen(false); }, style: { position: "fixed", inset: 0, zIndex: 999998 } }) : null;
      var panel = open ? e("div", {
        style: {
          ...
          padding: 12, fontSize: 13, zIndex: 999999, overflowY: "auto"
        }
      }, header,
    ...
    function WritingIndexView(props) {
      ...
      function load() {
        setBusy(true); setErr(null);
        props.getCwd().then(function (root) {
          return props.workspaceIndex(root).then(function (r) {
            if (r && r.ok) setData(r.value);
            else if (r && r.error) setErr(String(r.error));
            else setData(null);
            setBusy(false);
          });
        }).catch(function (x) { setErr(String((x && x.message) || x)); setBusy(false); });
      ...
      function openPreview(c) {
        ...
        props.getCwd().then(function (root) {
          return props.chapterText(root, c.id).then(function (r) {
            if (r && r.ok && r.value) setPreview({ meta: c, text: r.value.text, full: r.value, error: false });
            else setPreview({ meta: c, text: null, error: true });
          });
        }).catch(function () { setPreview({ meta: c, text: null, error: true }); });
      }
      function openFile(c) {
        props.getCwd().then(function (root) {
          var abs = String(root).replace(/[\\/]+$/, "") + "/" + c.path;
          props.openPath(abs).catch(function () {});
        }).catch(function () {});
      }
      ...
      if (!data) listEls.push(e("div", { style: { color: C.dim } }, busy ? "加载中…" : "（未初始化项目：对话中 novel_init，或侧边栏「写作」面板一键初始化）"));
        ...
        for (var g2 = 0; g2 < groups.length; g2++) {
          var grp = groups[g2];
          listEls.push(e("div", { key: "v" + grp.name, style: { color: C.dim, fontSize: 11, fontWeight: 700, letterSpacing: 1, margin: "10px 0 4px" } }, "◆ " + grp.name + " · " + grp.items.length + " 章"));
